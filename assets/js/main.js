$(document).ready(function () {
  ...
  $('#slotSearchEmployeeDiscountDay').load('./partials/searchEmployeeDiscountDay.php');
  ...
  $(function () {
    ...
    function initializeDateSelector(inputName, minDate) {
      ...
      $('input[name="' + inputName + '"]').on('apply.daterangepicker', function (
        ev,
        picker
      ) {
      ...
      $('input[name="' + inputName + '"]').on('cancel.daterangepicker', function (
        ev,
        picker
      ) {
        $(this).val('');
    ...
    if (
      ...
      if (validateCedula(cedulaEmpleado.trim())) {
        ...
        if (emailPattern.test(emailEmpleado)) {
          fechaInicioLaboral = moment(fechaInicioLaboral.trim(), 'DD/MM/YYYY').format('YYYY-MM-DD');
          insertDataEmployee(idTipoCodigo, idTipoContrato, idDepartamento, idJobTitle, cedulaEmpleado, nombreEmpleado,
            apellidoEmpleado, fechaInicioLaboral, telefonoEmpleado, direccionEmpleado, emailEmpleado, salary);
        } else {
  ...
  $('#actualizadatos').click(function () {
    ...
    if (
      ...
      if (validateCedula(cedulaEmpleado.trim())) {
        ...
        if (emailPattern.test(emailEmpleado)) {
          fechaInicioLaboral = moment(fechaInicioLaboral.trim(), 'DD/MM/YYYY').format('YYYY-MM-DD');
          updateDataEmployee(idEmployee, idTipoCodigo, idTipoContrato, idDepartamento, idJobTitle, cedulaEmpleado, nombreEmpleado,
            apellidoEmpleado, fechaInicioLaboral, telefonoEmpleado, direccionEmpleado, emailEmpleado, salary);
        } else {
          toastr.error("El correo es incorrecto", "MENSAJE");
        }
  ...
  $('#btnSendRequestPermissDateVacation').click(function () {
    ...
    if (idEmployee !== '' && idReason !== '' && rangeDate !== '' && observation !== '') {
      dates = rangeDate.split('-');
      startDate = moment(dates[0].trim(), 'DD/MM/YYYY').format('YYYY-MM-DD');
      endDate = moment(dates[1].trim(), 'DD/MM/YYYY').format('YYYY-MM-DD');
    ...
    if (idPeriod !== '' && startDate !== '' && endDate !== '' && earndDays !== '' &&
      ...
      if (startDateObj < endDateObj) {
        ...
        if (dateDifference >= oneYearMs - oneDayMs) {
          updatePeriodVacation(idPeriod, startDate, endDate, earndDays, balanceDays, workingDays, weekendDays, statePeriod);
        } else {
          toastr.error('El rango de fechas debe ser de 1 año menos 1 día', 'MENSAJE');
        }
      } else {
        toastr.error('La fecha de inicio debe ser anterior a la fecha final', 'MENSAJE');
      }
    } else {
      toastr.error('Ingrese todos los datos', 'MENSAJE');
    }
  ...
  $('#btnEmployeePermissBack').click(function () {
    ...
    if (datePermissBack !== '') {
      datePermissBack = moment(datePermissBack, 'DD/MM/YYYY').format('YYYY-MM-DD');

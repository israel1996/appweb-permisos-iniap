...
$table = "vw_permissDirector";
$id = 'id_permission';
$idEmployeeDirector = 0;
$idDepartament = 0;
$typeUser = 0;
...
if (isset($_SESSION['user'])) {
    ...
    $idEmployeeDirector = $user->getIdEmployee();
...
if ($idEmployeeDirector != 0) { // Asume que 0 es un valor no válido para un id real
    if ($where != "") {
        $where .= " AND ";
    } else {
        $where = "WHERE ";
    }
    $where .= "id_employee != :idEmployeeDirector";
    $params[':idEmployeeDirector'] = $idEmployeeDirector;
}
...
// Condición para incluir permisos de jefes de cualquier departamento
$whereBoss = "isBoss_employee = 1";

if ($where != "") {
    $where .= " OR ($whereBoss)";
} else {
    $where = "WHERE $whereBoss";
}
